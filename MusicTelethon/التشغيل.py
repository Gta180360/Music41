...
def ytsearch(query):
    try:
        ...
        return 0
async def ytdl(link):
    proc = await asyncio.create_subprocess_exec(        "yt-dlp",        "-g",        "-f",              "best[height<=?360][width<=?1280]",        f"{link}",        stdout=asyncio.subprocess.PIPE,        stderr=asyncio.subprocess.PIPE,    )
    stdout, stderr = await proc.communicate()
    if stdout:
        return 1, stdout.decode().split("\n")[0]
    else:
    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/06b651dc0d827b6887764.jpg",
                    caption=f"""
**🏷️ العنوان : [{songname}]({link})
💬 ايدي المحادثه : {chat_id}
🎧 طلب من : {m.from_user.mention}
💻 مبرمج السورس : [𐇮 ميططرون ](t.me/a150a)**
""",                )
            else:
    ...
    if replied:
        if replied.video or replied.document:
            ...
            if len(m.command) < 2:
                ...
                if pq == "360" or "480" or "360":
                    Q = int(pq)
                else:
                    Q = 360
                    await huehue.edit(                        "مسموح فقط بالدقه الأتيه :  360 ، 480 ، 360 \n ينزل الان الآن بدقة 360 بكسل   "                 )
